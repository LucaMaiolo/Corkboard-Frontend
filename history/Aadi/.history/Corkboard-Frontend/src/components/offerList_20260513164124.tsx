...
export const OfferList = ({
  ...
  const handleAccept = async (id: string): Promise<void> => {
    await fetch(`http://localhost:1339/offers/${id}/accept`, {
      method: "PUT",
      credentials: "include",
    });
  ...
  const handleDecline = async (id: string): Promise<void> => {
    await fetch(`http://localhost:1339/offers/${id}/decline`, {
      method: "PUT",
      credentials: "include",
    });
    setVersion((previous) => previous + 1);
        ...
        return (
          <li key={offer._id}>
            <OfferCard
              offer={offer}
              onAccept={
                isLister ? () => void handleAccept(offer._id) : undefined
              }
              onDecline={
                isLister ? () => void handleDecline(offer._id) : undefined
              }
            />
          </li>
        );
